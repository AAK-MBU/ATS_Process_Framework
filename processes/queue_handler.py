...
async def concurrent_add(
    ...
    # Sort items by entire JSON structure (all key-value pairs) before processing
    sorted_items = sorted(items, key=create_sort_key)
    logger.info(f"Processing {len(sorted_items)} items sorted by complete JSON structure")
